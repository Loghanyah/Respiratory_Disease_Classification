...
BATCH_SIZE = 32
NUM_CLASSES = 6 # Update as per your dataset
LR = 0.0001
EPOCHS = 20
IMG_SIZE = 224 # change to 128 or 64 or 224 for each experiment
EXP_NAME = f"Exp1_ImageSize_{IMG_SIZE}"
...
def train_model():
    ...
    for epoch in range(EPOCHS):
        ...
        if val_acc > best_acc:
            best_acc = val_acc
            torch.save(model.state_dict(), f"resnet50_{EXP_NAME}.pth")
...
# ----------------------------
def evaluate_test():
    model.load_state_dict(torch.load(f"resnet50_{EXP_NAME}.pth"))
    model.eval()
    ...
    print(f"Avg Inference Time per Batch: {avg_time:.6f} sec")

    cm = confusion_matrix(all_labels, all_preds)
    ConfusionMatrixDisplay(cm, display_labels=train_data.classes).plot(cmap=plt.cm.Blues)
    plt.show()
...
def plot_val_confusion_matrix(): 
    ...
    print("✔ Validation confusion matrix saved as 'val_confusion_matrix.png")

# ----------------------------
# RUN
# ----------------------------
train_model()
plot_val_confusion_matrix()
evaluate_test()